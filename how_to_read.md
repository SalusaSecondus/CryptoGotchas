...
* Negligibable Probability: Basically, as values (usually the security paramter) get sufficient large, the probability of something happen becomes some small you can (mostly) ignore it. A function \\(f\\) is negligible if for any polynomial \\(p\\), \\(|f(n)| < 1/p(n)\\) for all sufficiently large \\(n\\). You can think of negligible functions as getting really small at the rate of an inverse exponential
